...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  HAL_StatusTypeDef canStartResult;

  canStartResult = HAL_FDCAN_Start(&hfdcan1);

  if (canStartResult != HAL_OK)
  {
      Error_Handler();
  }
  ...
  while (1)
  {
      ...
      /* Heartbeat alle 500 ms */
      if ((now - heartbeatTimer) >= 500U)
      {
          heartbeatTimer = now;
          Can_SendHeartbeat();
      }
      ...
      /* Zusätzlich regelmäßig Status senden */
      if ((now - canTimer) >= 800U)
      {
          canTimer = now;
          Can_SendStatusFrame();
      }
